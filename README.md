...
Если не заданы оба параметра, бот попробует использовать единственный активный канал
из списка `/chats`.

Если ссылка канала не определена, кнопка «Подписаться на канал» не будет показана
(чтобы не уводить пользователя на главную страницу MAX).
